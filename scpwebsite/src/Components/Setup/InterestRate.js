...
export default function InterestRate() {
  ...
  //   useEffect(() => {
  //     console.log("HEHEHE", setupInterestRate);
  //   }, [setupInterestRate]);

  const fetchInterestRate = async () => {
    try {
      const response = await GetGetInterestRate();
      if (response) {
        setSetupInterestRate(response);
      }
    } catch (error) {
  ...
  const handleRateChange = (list, value) => {
    if (list) {
      setSetupInterestRate((prev) =>
        prev.map((item) =>
          item.rateCode === list.rateCode
            ? { ...item, rate: value, mark: "update" }
            : item
        )
      );
    }
  ...
  const handleSet = async () => {
    const updateData = [];
    setupInterestRate.forEach((item) => {
      const { rateCode, rate, mark } = item;
    ...
    setLoading(true);
    try {
      const response = await UpdateInterestRate(updateData);
  ...
  return (
    <>
      ...
      <div className="bg-prim d-flex justify-content-between">
        <h6 className="p-2">Interest Rate</h6>
      </div>
        ...
        <div className="mx-auto w-75 p-2 shadow border-top-sec rounded bg-light">
          <div className="table-responsive text-center">
            <table className="table table-sm table-hover table-striped table-light">
              ...
              <tbody>
                {setupInterestRate?.map((item, index) => (
                  <tr key={index}>
                    <td>{item.rateDesc || ""}</td>
                    <td>
